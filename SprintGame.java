...
public class SprintGame {
    ...
    void updateBubbleCount(int bubbles, boolean isPenalty) {
        ...
        if (isPenalty)
            ...
            bubbleCount = getBubbleCount() + bubbles;
        UpdateSharedPrefInt(Constants.SPRINT_BUBBLE_COUNT_KEY, bubbleCount);
    }

    int getCurrentScore() {
        return mContext.getSharedPreferences(Constants.COMMON_SHARED_PREF, Context.MODE_PRIVATE).
                getInt(Constants.SPRINT_CURRENT_SCORE_KEY, 1);
    }

    void updateCurrentScore(int score) {
        UpdateSharedPrefInt(Constants.SPRINT_CURRENT_SCORE_KEY, score);
    }

    int getHighScore() {
        return mContext.getSharedPreferences(Constants.COMMON_SHARED_PREF, Context.MODE_PRIVATE).
                getInt(Constants.SPRINT_HIGH_SCORE_KEY, 1);
    }

    void updateHighScore(int score) {
        UpdateSharedPrefInt(Constants.SPRINT_HIGH_SCORE_KEY, score);
    }
